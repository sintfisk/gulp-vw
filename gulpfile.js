...
gulp.task('css', function () {
  ...
  return gulp.src('./src/css/*.css') // 指定css文件夹下的所有后缀为.css的文件
    .pipe(postcss([autoprefixer(['iOS >= 7', 'Android >= 4.1'])])) //自动加上浏览器前缀
...
gulp.task('html', function () {
  gulp.src('./src/*.html') //指定当前文件夹下的所有html文件
    .pipe(minhtml()) //进行压缩
...
gulp.task('default', function () {
  //把任务串联起来
  gulp.start('js', 'css', 'less', 'img', 'html', 'movelibs');
  ...
  gulp.watch([path.html, path.js, path.css]).on("change", function () {
    gulp.start('js', 'less', 'css', 'img', 'html', 'movelibs');
    browserSync.reload();
  });
});